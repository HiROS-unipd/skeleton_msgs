...
Pose::Pose(const Vector3 &t_position, const Quaternion &t_orientation)
    : position(t_position), orientation(t_orientation) {}
...
Velocity::Velocity(const Vector3 &t_linear, const Vector3 &t_angular)
    : linear(t_linear), angular(t_angular) {}
...
// KinematicState
KinematicState::KinematicState(const Pose &t_pose, const Velocity &t_velocity,
                               const Acceleration &t_acceleration)
    : pose(t_pose), velocity(t_velocity), acceleration(t_acceleration) {}
...
// Box
Box::Box(const KinematicState &t_center, const double &t_height,
         const double &t_length, const double &t_width)
    : center(t_center), height(t_height), length(t_length), width(t_width) {}
...
// Marker
Marker::Marker(const int &t_id, const std::string &t_name,
               const double &t_confidence, const KinematicState &t_center)
    : id(t_id), name(t_name), confidence(t_confidence), center(t_center) {}

Marker::Marker(const int &t_id, const double &t_confidence,
...
// Link
Link::Link(const int &t_id, const std::string &t_name,
           const int &t_parent_marker, const int &t_child_marker,
           const double &t_confidence, const KinematicState &t_center)
    : id(t_id),
      name(t_name),
...
Link::Link(const int &t_id, const int &t_parent_marker,
...
const Marker &Skeleton::getMarker(const int &t_id) const {
  ...
  throw std::out_of_range("Marker " + std::to_string(t_id) +
...
bool Skeleton::removeMarker(const int &t_id) {
  ...
  markers.erase(std::remove_if(markers.begin(), markers.end(),
                               [&](const Marker &m) { return m.id == t_id; }));
  return true;
}
...
const Link &Skeleton::getLink(const int &t_id) const {
  ...
  throw std::out_of_range("Link " + std::to_string(t_id) +
                          " not present in Skeleton " + std::to_string(id));
}
...
// SkeletonGroup
SkeletonGroup::SkeletonGroup(const double &t_time, const std::string &t_frame,
                             const std::vector<Skeleton> &t_skeletons)
    : time(t_time), frame(t_frame), skeletons(t_skeletons) {}
