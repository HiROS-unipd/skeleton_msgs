...
struct Pose {
  Pose(
      ...
      const Quaternion& t_orientation =
          Quaternion(std::numeric_limits<double>::quiet_NaN(),
                     std::numeric_limits<double>::quiet_NaN(),
                     std::numeric_limits<double>::quiet_NaN(),
                     std::numeric_limits<double>::quiet_NaN()));
...
// Velocity
struct Velocity {
  Velocity(const Vector3& t_linear =
               Vector3(std::numeric_limits<double>::quiet_NaN(),
                       std::numeric_limits<double>::quiet_NaN(),
                       std::numeric_limits<double>::quiet_NaN()),
           const Vector3& t_angular =
               Vector3(std::numeric_limits<double>::quiet_NaN(),
                       std::numeric_limits<double>::quiet_NaN(),
                       std::numeric_limits<double>::quiet_NaN()));
...
// Marker
struct Marker {
  Marker(const int& t_id = -1, const std::string& t_name = "",
...
// Link
struct Link {
  Link(const int& t_id = -1, const std::string& t_name = "",
       const int& t_parent_marker = -1, const int& t_child_marker = -1,
       const double& t_confidence = std::numeric_limits<double>::quiet_NaN(),
       const KinematicState& t_center = KinematicState());
  Link(const int& t_id, const int& t_parent_marker = -1,
...
struct SkeletonGroup {
  SkeletonGroup(
      const double& t_time = std::numeric_limits<double>::quiet_NaN(),
